...
#   if (!("nb" %in% nopts)) {
#     opts$nb <- 150L
#   } else if (!is.numeric(opts$nb) || opts$nb <= 2L) {
#     stop("nb needs to be a finite positive number greater than or equal to 2.")
#   }
#
#   if (!("ng" %in% nopts)) {
#     opts$ng <- 50L
#   } else if (!is.numeric(opts$ng) || opts$ng <= 2L) {
#     stop("ng needs to be a finite positive number greater than or equal to 2.")
#   }
#
#   if (is.null(tol)) tol <- sqrt(.Machine$double.eps)
#
...
#   while (!converged) {
#     m <- m + 1L
#     bArr <- exp(seq(log(bLo), log(bHi), length.out = opts$nb))
#     gArr <- 1 + exp(seq(log(gLo), log(gHi), length.out = opts$ng))
#     bmu <- bcv <- double(opts$ng)
#     muArr <- (outer(gArr, bArr, mugb) - mu) ^ 2
#     cvArr <- (outer(gArr, bArr, cvgb) - cv) ^ 2
#     bmu <- bArr[apply(muArr, 1L, which.min)]
#     bcv <- bArr[apply(cvArr, 1L, which.min)]
#     bdiff <- bmu - bcv
#     if (any(bdiff != 0)) {
...
#       bHi <- bArr[min(opts$nb, lowerUpb)]
#       gLo <- gArr[max(1L, upperLog)] - 1L
#       gHi <- gArr[min(opts$ng, lowerUpg)] - 1L
