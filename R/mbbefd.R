...
dmbR <- function(x, g, b, log = FALSE) {
  ...
  gm1 <- g - 1
  gb <- g * b
  ret <- ifelse(g < 1 | b < 0 | is.nan(x), NaN,
                ifelse(g == 1 | b == 0 | x < 0 | x >= 1, 0,
                       ifelse(b == 1, gm1 / (1 + gm1 * x) ^ 2,
                              ifelse(gb == 1, -log(b) * b ^ x,
                                     ((b - 1) * gm1 * log(b) * b ^ (1 - x)) /
                                       (gm1 * b ^ (1 - x) + (1 - gb)) ^ 2
                              )
                       )
  ...
  # Note to self: ask mbbefd maintainers why they return 1/g for x == 1.
  # (AA: 2024-02-28)
